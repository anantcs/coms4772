...
def sparsenet(IMAGES, patch_dim=64, neurons=128, lambdav=0.1, eta=6.0, num_trials=100, batch_size=100, border=4, inference='fista'):
  """
  N: # Inputs
  M: # Outputs
  ...
  (num_images, imsize, imsize) = np.shape(IMAGES)
  ...
  for t in range(num_trials):
    ...
    for i in range(batch_size):
      ...
      # print(imi-1)
      np.reshape(IMAGES[imi-1, r:r+sz, c:c+sz], patch_dim, 1)
      I[:,i] = np.reshape(IMAGES[imi-1, r:r+sz, c:c+sz], patch_dim, 1)
